...
# حط المفتاح الجديد اللي جبته من "New Project" هنا
...
try:
    ...
    st.error("Error setting up the API")
...
if prompt:
    with st.chat_message("assistant"):
        try:
            # تجربة إرسال بسيطة جداً
            response = model.generate_content(prompt)
            st.write(response.text)
        except Exception as e:
            # لو فشل هيقولنا ليه بالظبط (مثلاً: مفتاح منتهي، أو دولة غير مدعومة)
